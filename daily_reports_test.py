#!/usr/bin/env python3
"""
Daily Reports and Statistics Bug Testing
...
class DailyReportsStatisticsTester:
    def __init__(self, base_url="https://agency-billing-pro.preview.emergentagent.com"):
    ...
    def test_daily_reports_and_statistics_bug(self):
        """Test Daily Reports and Statistics bug as requested in review"""
        print(f"\n📊 TESTING DAILY REPORTS AND STATISTICS BUG (Review Request)...")
        print(f"   Testing report generation functionality with superadmin@sanhaja.com / super123")
        ...
        if success:
            ...
            if len(reports_data) > 0:
                print(f"   Sample report structure: {list(reports_data[0].keys())}")
                for i, report in enumerate(reports_data[:3]):  # Show first 3 reports
                    print(f"   Report {i+1}: Date={report.get('date', 'N/A')}, Status={report.get('status', 'N/A')}")
            else:
                print(f"   No existing reports found")
        ...
        sample_report_data = {
            ...
            "notes": "تقرير يومي تجريبي للاختبار - Daily test report"
        }
        ...
        if success:
            ...
            print(f"   Created report ID: {create_response.get('id', 'N/A')}")
            created_report_id = create_response.get('id')
        else:
            print(f"   ❌ Failed to create daily report")
        ...
        if created_report_id:
            ...
            if success:
                ...
                print(f"   ❌ Report approval failed")
        else:
            print(f"   ⚠️  Skipping approval test - no report ID available")
            results['approve_daily_report'] = False
        
        # Step 5: Test GET /api/reports/sales with date parameters
        print(f"\n   5. Testing GET /api/reports/sales with date parameters...")
        ...
        success, daily_sales = self.run_test(
            ...
            "GET",
            f"reports/sales?start_date={start_date.isoformat()}&end_date={end_date.isoformat()}&report_type=daily",
            200
        )
        ...
        success, monthly_sales = self.run_test(
            ...
            "GET",
            f"reports/sales?start_date={start_date.isoformat()}&end_date={end_date.isoformat()}&report_type=monthly",
            200
        )
        ...
        if success:
            ...
            if dashboard_data.get('today_income', 0) >= 0:
                print(f"   ✅ Today's income calculation working")
                results['today_income_calculation'] = True
            else:
                print(f"   ❌ Today's income calculation issue")
                results['today_income_calculation'] = False
            ...
            for invoice in invoices_data:
                ...
                if created_at:
                    try:
                        ...
                        print(f"   ⚠️  Date parsing error for invoice: {e}")
        ...
        # Step 8: Test date format verification in API calls
        print(f"\n   8. Testing date format verification...")
        
        # Test with valid date format
        valid_date = datetime.now(timezone.utc).isoformat()
        success, response = self.run_test(
            "Sales Report - Valid Date Format",
            "GET",
            f"reports/sales?start_date={valid_date}&end_date={valid_date}&report_type=daily",
            200
        )
        results['valid_date_format'] = success
        
        if success:
            print(f"   ✅ Valid date format accepted")
        ...
        # Step 9: Test with specific date ranges
        print(f"\n   9. Testing with specific date ranges...")
        
        # Test with last week
        last_week_start = (datetime.now(timezone.utc) - timedelta(days=7)).isoformat()
        last_week_end = datetime.now(timezone.utc).isoformat()
        
        success, last_week_report = self.run_test(
            "Sales Report - Last Week Range",
            "GET",
        ...
        last_month_start = (datetime.now(timezone.utc) - timedelta(days=30)).isoformat()
        last_month_end = datetime.now(timezone.utc).isoformat()
        
        success, last_month_report = self.run_test(
            "Sales Report - Last Month Range",
            "GET",
        ...
        print(f"\n   10. SUMMARY OF DAILY REPORTS AND STATISTICS TESTING:")
...
def main():
    print("📊 Daily Reports and Statistics Bug Testing")
    print("Testing report generation functionality as requested in review")
    print("=" * 80)
    
    tester = DailyReportsStatisticsTester()
    
    # Run the daily reports and statistics bug testing
    results = tester.test_daily_reports_and_statistics_bug()
